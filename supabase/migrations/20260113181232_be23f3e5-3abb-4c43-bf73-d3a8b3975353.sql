-- Messaging: permanently fix potential RLS recursion by ensuring helper functions
-- bypass row-level security and rebuilding policies to reference only those helpers.

-- 1) Drop existing policies on messaging tables (safe reset)
DO $$
DECLARE r record;
BEGIN
  FOR r IN (
...
-- 2) SECURITY DEFINER helpers (explicitly disable row_security inside the function)
...
-- Make sure authenticated users can execute these helpers (needed for RLS evaluation)
GRANT EXECUTE ON FUNCTION public.is_chat_member(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_chat_admin(uuid, uuid) TO authenticated;

-- 3) Recreate RLS policies (no self-referencing subqueries)
...
-- Prevent client-side inserting participants (memberships are created via server-side RPCs)
